...
int mstringValid(const mstring * str)
{
    ...
    if ((str->canarybuf ^ (uintptr_t) str->buf) == bufferkey && (str->canarylen ^ (uintptr_t) str->len) == lengthkey && (*bufterminator == (bufferkey ^ (uintptr_t) str)))
        return 1;
...
void mstringNew(mstring * str, size_t len)
{
    ...
    if (str == NULL)
        mstringFatal(str, "null pointer passed to mstringNew()");
    ...
    if (!str->buf)
        mstringFatal(str, "malloc failed in mstringNew()");
...
void mstringDelete(mstring * str)
{
    if (mstringValid(str))
        ...
        mstringComplain(str, "deleting invalid mstring");
...
void mstringSet(mstring * str, void *src, size_t len)
{
    ...
    if (len > str->len)
        mstringFatal(str, "excessive length in mstringSet()");

    if (mstringValid(str)) {
        memcpy(str->buf, src, len);
        str->buf[len] = 0;
    }                           // safe: may go into the reserved spot: won't truncate

    else
        mstringFatal(str, "invalid mstring in mstringSet()");
}
...
{
    if (!mstringValid(str))
        mstringFatal(str, "invalid source in mstringAppend()");
...
void mstringGrow(mstring * str, size_t newlen)
{
    ...
    size_t tmplen;
    if (!mstringValid(str))
        mstringFatal(str, "invalid source in mstringGrow()");
    if (newlen == str->len)
        return;                 // nothing to see here
    if (newlen < str->len)
        mstringFatal(str, "trying to shrink in mstringGrow()");
    tmplen = str->len;
    tmp = (char *) malloc(tmplen);
    if (!tmp)
        mstringFatal(str, "malloc failure in mstringGrow()");
...
void mstringDebug(const mstring * str)
{
    ...
    fprintf(stderr, "canarybuf:\t0x%lx / 0x%lx\n", str->canarybuf, (uintptr_t) str->canarybuf ^ (uintptr_t) str->buf);
    fprintf(stderr, "buf:\t\t%p\n", str->buf);
    fprintf(stderr, "len:\t\t%lu\n", (ulong) str->len);
    fprintf(stderr, "canarylen:\t0x%lx / 0x%lx\n", str->canarylen, (ulong) str->canarylen ^ (ulong) str->len);
    if (mstringValid(str))
        fprintf(stderr, "bufterminator:\t\t0x%lx\n", (ulong) * bufterminator);
    else
        fprintf(stderr, "bufterminator: not printed: bad deref\n");
